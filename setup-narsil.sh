...
if [ -n "$SECOND_USER" ]; then
    ...
    # Rimuove g+w dalle directory di completamento zsh mantenendo g+w sul resto di Homebrew
    print_step "Correggendo permessi directory completamento zsh..."
    chmod g-w /opt/homebrew/share/zsh /opt/homebrew/share/zsh/site-functions 2>/dev/null || true
    # Fix anche zsh-autocomplete se installato (la versione cambia)
